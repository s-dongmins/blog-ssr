...
export async function commentGET(key: string) {
    ...
    const items: Comment[] = json.Items.sort(
        (a: Comment, b: Comment) => b.datetime - a.datetime
    );
    return items;
}
...
export async function postSCAN(page: number = 1, tag: string = "", fullscan: boolean = false) { // postSCAN(1, "tag,list,man")
    const response = await fetch(apiURL + "post");
    const json = await response.json();
    const items: Post[] = json.Items.sort(
        (a: Post, b: Post) => b.datetime - a.datetime
    );
    if (fullscan) {
        return items;
    }
